...
// Manually sets a channel's commission rate and delivery-charge rate —
// GrubCenter never exposes either figure itself, so neither has a sync/import
// source and both must be entered here. Keyed by channel name (not id): the
...
export async function POST(req: NextRequest) {
  ...
  const { channel, commissionRate, deliveryChargeRate } = (body ?? {}) as {
    channel?: unknown;
  ...
  try {
    const updated = await prisma.channel.update({
      where: { name },
      data: { commissionRate: round2(rate), deliveryChargeRate: round2(dcRate) },
    });
